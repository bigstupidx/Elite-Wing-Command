...
class TerrainComposer extends EditorWindow 
{
    ...
    */
          	...
          	if(GUILayout.Button(GUIContent(button_export,tooltip_text),GUILayout.Width(52),GUILayout.Height(32)))
        	{
        		if (!script2)
        		{
	        		...
	        		if (script.button_export)
	        		{
	        			...
	        			else if ((script.tree_output || script.grass_output) && !script.meshcapture_tool){script.button_generate_text = "Export Map";}
	        			else
	        			{
	    ...
	    if (global_script.settings.download_display)
	    {
			...
			*/
			...
			if (global_script.settings.downloading2 == 1) {
				...
				if (File.Exists(Application.dataPath+"/TerrainComposer Examples/Projects/version1.1.txt")) {
					...
					if (File.Exists(Application.dataPath+"/TerrainComposer/Download/TerrainComposer_Examples.unitypackage")) {
						if (GUILayout.Button("Import",GUILayout.Width(100))) {
							AssetDatabase.ImportPackage(Application.dataPath+"/TerrainComposer/Download/TerrainComposer_Examples.unitypackage",true);
						}
					}
					else if (GUILayout.Button("Download",GUILayout.Width(100))) {
						global_script.settings.download2 = new WWW("http://www.terraincomposer.com/TerrainComposer_Examples.unitypackage");  
		    ...
		    gui_changed_old = GUI.changed;
		   	...
		   	if (GUI.changed) {
		   		if (global_script.settings.example_tiles*Mathf.Pow(2,7+global_script.settings.example_resolution) > 8192) {
		   			do {
		   				--global_script.settings.example_tiles;
		   			}
		   	...
		   	if (GUI.changed) {
		   		if (global_script.settings.example_tiles*Mathf.Pow(2,7+global_script.settings.example_resolution) > 8192) {
		   			do {
		   				--global_script.settings.example_resolution;
		   			}
		   			while (global_script.settings.example_tiles*Mathf.Pow(2,7+global_script.settings.example_resolution) > 8192);
		   		}
		   	}
        ...
        if (!script2 || script.generate_auto)
        {
        	...
        	if (script.heightmap_output || script.color_output || script.splat_output || script.tree_output || script.grass_output || script.object_output || script.meshcapture_tool)
        	{
	        	if (GUILayout.Button(script.button_generate_text,GUILayout.Width(100),GUILayout.Height(20)) || (key.control && key.keyCode == KeyCode.G))
	        	{
	        		if (!script.meshcapture_tool)
	        		{
	        			if (script.button_export && !script.color_output && !script.splat_output)
	        			{
	        				if (script.heightmap_output) {
    ...
    function generate_stop()
    {
		...
		if (create_pass != -1) {++create_pass;create_example(String.Empty,false);}
	}
	        		...
	        		if (current_terrain.data_foldout || script.settings.tabs)
	        		{
		        		...
		        		if (current_terrain.resolution_foldout)
		        		{
			        		...
			        		*/
			        		...
			        		EditorGUILayout.BeginHorizontal();
			        		GUILayout.Space(60+space);
			        		EditorGUILayout.LabelField("Detail Resolution",GUILayout.Width(135+214));
		        			...
		        			if (global_script.settings.tooltip_mode != 0)
					        {
			        			...
			        			if (current_terrain.rtp_script) {
			        				...
			        				if (script.rtpLod_script) {
				        				if (script.rtpLod_script.RTP_NORMALGLOBAL_FIRST || script.rtpLod_script.RTP_NORMALGLOBAL_ADD) {
				        					gui_changed_old = GUI.changed;
			        						GUI.changed = false;
		        		...
		        		if (current_terrain.detail_foldout)
		        		{
		        			...
		        			if (GUILayout.Button(GUIContent("-",tooltip_text),GUILayout.Width(25)))
		        			{
		        				if (key.control) {
			        				...
			        				{
			        					#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        								Undo.RecordObject(script,"Detail Erase");
							        	#else
	...
	function toggle_tree_output()
	{
		...
		// {
	        ...
	        else
	        {
	        	...
	        	{
	        		global_script.settings.example_buttons = 4;
	        		if (!script.heightmap_output && script.button_export){script.button_generate_text = "Export Map";}
	...
	*/
	...
	function create_terrain(preterrain: terrain_class,length: int,start_number: int,name_number: int,terrain_path: String,terrain_parent: Transform)
	{
		...
		*/
		...
		for (var count_terrain1: int = 0;count_terrain1 < length;++count_terrain1)
		{
		    ...
		    script.set_terrain_parameters(script.terrains[count_terrain1+start_number-1]);
		    ...
		    script.set_terrain_splat_textures(script.terrains[count_terrain1+start_number-1],script.terrains[count_terrain1+start_number-1]);
		    ...
		    if (script.terrains[count_terrain1+start_number-1].rtp_script) {script.assign_rtp_single(script.terrains[count_terrain1+start_number-1]);}
		    assign_terrain_splat_alpha(script.terrains[count_terrain1+start_number-1]);
		}
		AssetDatabase.Refresh();
		if (select_window) {select_window.Repaint();}
	}
	...
	function check_terrains()
	{
		for (var count_terrain1: int = 0;count_terrain1 < script.terrains.Count;++count_terrain1)
		{
			...
			assign_terrain_splat_alpha(script.terrains[count_terrain1]);
		}
	}
	    	...
	    	if (global_script.settings.download2) {
	    		if (global_script.settings.download2.isDone) {
	        		...
	        		if (!global_script.settings.download2.error) {
	        			File.WriteAllBytes(Application.dataPath+"/TerrainComposer/Download/TerrainComposer_Examples.unitypackage",global_script.settings.download2.bytes);
	        		}
	        	}
	        } 
	        else {
	...
	function slice_terrains()
	{
		...
		for (count_terrain1 = 0;count_terrain1 < script.terrains.Count;++count_terrain1)
		{
			...
			{
				this.ShowNotification(GUIContent("Detail resolution of "+script.terrains[count_terrain1].name+" is 32, this cannot be made smaller, read console suggestion"));
			...
			for (var row: float = 0;row < 2;++row)
			{
				for (var count_y: int = 0;count_y < script.terrains[0].tiles.y;++count_y)
				{
					...
					for (var count_terrain2: float = 0;count_terrain2 < 2;++count_terrain2)
					{
						...
						assign_terrain_splat_alpha(terrain_new);
	...
	function create_example(path: String,load_only: boolean)
	{
		if (create_pass == -1) {
			if (global_script.settings.example_terrain != global_script.settings.example_terrain_old1) {
	...
	function load_example_terrain(load_only: boolean)
	{
		if (global_script.settings.example_terrain == 0) {
			create_example("Assets/TerrainComposer Examples/Projects/Procedural_Mountains.prefab",load_only);
		}
		else if (global_script.settings.example_terrain == 1) {
			create_example("Assets/TerrainComposer Examples/Projects/Procedural_Canyons.prefab",load_only);
		}
		else if (global_script.settings.example_terrain == 2) {
			create_example("Assets/TerrainComposer Examples/Projects/Procedural_Plateaus.prefab",load_only);
		}
		else if (global_script.settings.example_terrain == 3) {
			create_example("Assets/TerrainComposer Examples/Projects/Procedural_Islands.prefab",load_only);
		}
		else if (global_script.settings.example_terrain == 4) {
			create_example("Assets/TerrainComposer Examples/Projects/Island_Example.prefab",load_only);
		}
	}

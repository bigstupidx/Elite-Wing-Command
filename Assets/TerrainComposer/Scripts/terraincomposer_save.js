				...
				else 
				{
					...
					else
					{
						...
						if (!splatmap) {
							for (count_value = 0;count_value < current_layer.splat_output.splat.Count;++count_value)
							{
								preterrain.splat[current_layer.splat_output.splat[count_value]] += (current_layer.splat_output.splat_calc[count_value]*subfilter_value*filter_strength);
							}
						}
						else {
							for (count_value = 0;count_value < 4;++count_value)
	...
	for (var count_terrain: int = 0;count_terrain < terrains.Count;++count_terrain) {
		...
		if (active) {
			...
			if (!terrains[count_terrain].rtp_script) {
				terrains[count_terrain].rtp_script = UnityEngineInternal.APIUpdaterRuntimeServices.AddComponent(terrains[count_terrain].terrain.gameObject, "Assets/TerrainComposer/Scripts/terraincomposer_save.js(7331,70)", "ReliefTerrain");
...
function assign_rtp_single(terrain1: terrain_class)
{
	...
	if (!terrain1.rtp_script) {
		terrain1.rtp_script = UnityEngineInternal.APIUpdaterRuntimeServices.AddComponent(terrain1.terrain.gameObject, "Assets/TerrainComposer/Scripts/terraincomposer_save.js(7357,39)", "ReliefTerrain");
	}
}

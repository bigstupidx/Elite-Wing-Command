			...
			else
			{
				if (splat_output) {
					if (!settings.runtime_create_terrain) {
						...
						for (count_value = 0;count_value < preterrain.splat_length;++count_value) {
							alphamap[map_y,map_x,count_value] = preterrain.splat_layer[count_value];
							preterrain.splat_layer[count_value] = 0;
						}
					}
				}
	...
	switch (filter.output)
	{
		...
		case condition_output_enum.add:
			if (current_layer.output == layer_output_enum.heightmap){filter_value += filter_input*filter.strength*subfilter_value;}
				else {filter_value += filter_input*filter.strength;}
			...
			if (current_layer.output == layer_output_enum.color)
			{
				for (count_value = 0;count_value < 3;++count_value)
				{
					preterrain.color[count_value] += color0[count_value]*(current_layer.strength*filter.strength*subfilter_value);
				}
			}
			if (current_layer.output == layer_output_enum.splat)
			{
				if (!splatmap) {
					...
					for (count_value = 0;count_value < 4;++count_value)
					{
						...
						preterrain.splat[current_layer.splat_output.splat[count_value]] += color1[count_value]*(current_layer.strength*filter.strength*subfilter_value);
					}
				}
			}
			else if (current_layer.output == layer_output_enum.grass)
		...
		case condition_output_enum.change:
			if (current_layer.output == layer_output_enum.heightmap)
			{
				if (filter.change_mode == change_mode_enum.filter)
				{
					filter_value = (filter_value*(1-(subfilter_value*filter.strength)))+(filter_input*(subfilter_value*filter.strength));
				}
				else
				{
					if (filter.combine) {filter_combine = filter_value-filter_combine_start;return;}
					heights[heightmap_y,heightmap_x] = ((heights[heightmap_y,heightmap_x]+filter_value)*(1-(subfilter_value*filter.strength)))+(filter_input*(subfilter_value*filter.strength));
				}
			}
			...
			if (current_layer.output == layer_output_enum.color)
			{
				if (filter.type == condition_type_enum.Current)
				{
					if (filter.change_mode == change_mode_enum.filter)
					{
						for(count_value = 0;count_value < 3;++count_value)
						{
							preterrain.color[count_value] = preterrain.color[count_value]-(preterrain.color[count_value]*(1-subfilter_value)*current_layer.strength*filter.strength);
						}
					}
					else
					{
						for(count_value = 0;count_value < 3;++count_value)
						{
							preterrain.color_layer[count_value] = preterrain.color_layer[count_value]-(preterrain.color_layer[count_value]*(1-subfilter_value)*current_layer.strength*filter.strength);
						}
					}
				}
				else 
				{
					for(count_value = 0;count_value < 3;++count_value)
					{
						preterrain.color_layer[count_value] = (preterrain.color_layer[count_value]*(1-(subfilter_value*filter.strength*current_layer.strength)))+(color0[count_value]*subfilter_value*filter.strength*current_layer.strength);
					}
				}
			}
			if (current_layer.output == layer_output_enum.splat)
				...
				else 
				{
					if (filter.change_mode == change_mode_enum.filter)
					{
						if (!splatmap) {
							...
							{
								if (count_value > preterrain.splat.Length-1) {break;}
								preterrain.splat[current_layer.splat_output.splat[count_value]] = (preterrain.splat[current_layer.splat_output.splat[count_value]]*(1-(filter.strength*current_layer.strength*subfilter_value)))+color1[count_value]*(current_layer.strength*filter.strength*subfilter_value);
							}
						}
					}
					else
		...
		case condition_output_enum.divide:
			...
			if (current_layer.output == layer_output_enum.color)
			{
				for(count_value = 0;count_value < 3;++count_value)
				{
					if (color0[count_value]*(current_layer.strength*filter.strength*subfilter_value) != 0){preterrain.color[count_value] = preterrain.color[count_value]/(color0[count_value]*(current_layer.strength*filter.strength*subfilter_value));}
				}
			}
			if (current_layer.output == layer_output_enum.splat)
			{
				if (!splatmap) {
					...
					{
						if (count_value > preterrain.splat.Length-1) {break;}
						if (color1[count_value]*(current_layer.strength*filter.strength*subfilter_value) != 0) {
							preterrain.splat[current_layer.splat_output.splat[count_value]] /= color1[count_value]*(current_layer.strength*filter.strength*subfilter_value);
						}
					}
				}
		...
		case condition_output_enum.average:
			...
			if (current_layer.output == layer_output_enum.color)
			{
				for(count_value = 0;count_value < 3;++count_value)
				{
					preterrain.color[count_value] += (color0[0]*(current_layer.strength*filter.strength*subfilter_value))/current_layer.prefilter.filter_index.Count;
				}
			}
			if (current_layer.output == layer_output_enum.splat)
			{
				if (!splatmap) {
					for (count_value = 0;count_value < current_layer.splat_output.splat.Count;++count_value)
					{
						preterrain.splat[current_layer.splat_output.splat[count_value]] += (current_layer.splat_output.splat_calc[count_value]*(current_layer.strength*filter.strength*subfilter_value))/current_layer.prefilter.filter_index.Count;
					}
				}
				else {
					for (count_value = 0;count_value < 4;++count_value)
					{
						if (count_value > preterrain.splat.Length-1) {break;}
						preterrain.splat[current_layer.splat_output.splat[count_value]] += color1[count_value]*(current_layer.strength*filter.strength*subfilter_value)/current_layer.prefilter.filter_index.Count;
					}
				}
			}
			else if (current_layer.output == layer_output_enum.grass)
		...
		case condition_output_enum.difference:
			...
			if (current_layer.output == layer_output_enum.color)
			{
				for(count_value = 0;count_value < 3;++count_value)
				{
					preterrain.color[count_value] = Mathf.Abs(preterrain.color[count_value]-(color0[0]*(current_layer.strength*filter.strength*subfilter_value)));
				}
			}
			if (current_layer.output == layer_output_enum.splat)
			{
				if (!splatmap) {
					for (count_value = 0;count_value < current_layer.splat_output.splat.Count;++count_value)
					{
						preterrain.splat[current_layer.splat_output.splat[count_value]] = Mathf.Abs(preterrain.splat[count_value]-(current_layer.splat_output.splat_calc[count_value]*(current_layer.strength*filter.strength*subfilter_value)));
					}
				}
				else {
					for (count_value = 0;count_value < 4;++count_value)
					{
						if (count_value > preterrain.splat.Length-1) {break;}
						preterrain.splat[current_layer.splat_output.splat[count_value]] += Mathf.Abs(preterrain.splat[count_value]-(color1[count_value]*(current_layer.strength*filter.strength*subfilter_value)));
					}
				}
			}
			else if (current_layer.output == layer_output_enum.grass)
...
function calc_raw_value(raw: raw_class,local_x: float,local_y: float)
{
	...
	var index1: int = (h_local_y*(raw_files[raw.file_index[raw.raw_number]].resolution.x)*2)+(h_local_x*2);
		...
		if (preimage.precolor_range.color_range.Count > 0)
		{
			...
			for (count_color_range = 0;count_color_range < preimage.precolor_range.color_range.Count;++count_color_range)
			{
				...
				else 
				{
					...
					{
						if (preimage.select_mode == select_mode_enum.free) {
							preimage.output_pos = 1-preimage.precolor_range.color_range[count_color_range].precurve.curve.Evaluate(calc_color_pos(color1,color_start,color_end));
							preimage.output = true;
						}
						else {
							preimage.output_pos = output;
...
function stitch_splatmap()
{
	...
	for (var count_x: int = 0;count_x < terrains[0].tiles.x;++count_x)
	{
		for (var count_y: int = 0;count_y < terrains[0].tiles.y;++count_y)
		{
			...
			if (count_x != 0)
			{
				resolution = terrains[count_y+((count_x-1)*terrains[0].tiles.y)].terrain.terrainData.alphamapResolution;
			    if (terrains[terrain_index].terrain.terrainData.alphamapResolution != resolution){continue;}
				
				map = terrains[count_y+((count_x-1)*terrains[0].tiles.y)].terrain.terrainData.GetAlphamaps(resolution-1,0,1,resolution);
...
function smooth_terrain(preterrain1: terrain_class,strength: float)
{
	...
	for (var count_strength: int = 0;count_strength < smooth_tool_repeat;++count_strength)
	{
		...
		{
			for (var x: int = 0;x < heightmap_resolution;++x)
			{
				point1 = heights[x,y];
				...
				if (texture_tool.precolor_range.color_range_value.active[count_color_range])
				{
					...
					else if (color_in_range(pixel,color_start,color_end))
					{
					 	if (!texture_tool.precolor_range.color_range[count_color_range].invert) {
					 		if (previewMode & 1) {pixel = choose_color(count_color_range,texture_tool.precolor_range.color_range[count_color_range].precurve.curve.Evaluate(calc_color_pos(pixel,color_start,color_end)));} 
					 		in_range = true;
					 	} 
					}
					else if (texture_tool.precolor_range.color_range[count_color_range].invert) {
						in_range = true;
						if (previewMode & 1) {pixel = choose_color(count_color_range,1-texture_tool.precolor_range.color_range[count_color_range].precurve.curve.Evaluate(calc_color_pos(pixel,color_start,color_end)));} 
					}
				}
			}
			if (!in_range){
	...
	function set_raw_auto_scale(preterrain1: terrain_class,area: Rect,raw_files: List.<raw_file_class>,raw_number: int)
	{
		if (raw_number < file_index.Count)
		{
			if (raw_files[file_index[raw_number]].assigned && preterrain1)
			{
				...
				{
					conversion_step.x = (preterrain1.terrain.terrainData.size.x*preterrain1.tiles.x)/(raw_files[file_index[raw_number]].resolution.x);
					conversion_step.y = (preterrain1.terrain.terrainData.size.z*preterrain1.tiles.y)/(raw_files[file_index[raw_number]].resolution.y);
				}
			}
		}
	}
